    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
        <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-sm">
          <span className="text-slate-500 font-semibold">Total Reports</span>
          <p className="text-2xl font-black text-slate-900 mt-1">{metrics.total_reports}</p>
        </div>
        <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 shadow-sm">
          ...
          <p className="text-2xl font-black text-rose-700 mt-1">{metrics.critical_count}</p>
        </div>
        <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-200 shadow-sm">
          ...
          <p className="text-2xl font-black text-indigo-700 mt-1">{metrics.clusters_formed}</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-sm">
          <span className="text-slate-500 font-semibold">Duplicates Saved</span>
          <p className="text-2xl font-black text-emerald-700 mt-1">{metrics.duplicate_reports_merged}</p>
        </div>
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 shadow-sm">
          ...
          <p className="text-2xl font-black text-emerald-800 mt-1">{metrics.citizen_satisfaction_rate}%</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-sm">
          <span className="text-slate-500 font-semibold">SLA Compliance</span>
          <p className="text-2xl font-black text-slate-900 mt-1">{metrics.sla_compliance_rate}%</p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">Geospatial Civic Hazard Hotspot Map</h3>
              <p className="text-xs text-slate-500">Islamabad & Rawalpindi Metropolitan Region</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-xl border border-slate-200 text-xs">
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                selectedFilter === 'all' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                selectedFilter === 'critical' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Critical Risks Only
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                selectedFilter === 'clusters' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Proximity Clusters
            </button>
        ...
        {/* Map Container */}
        <div className="h-[420px] w-full rounded-2xl overflow-hidden border border-slate-200 relative z-10 shadow-inner bg-slate-100">
          <MapContainer
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-xs">
        ...
        <div className="p-6 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">Incident Category Distribution</h3>
          </div>
                ...
                return (
                  <div key={cat} className="space-y-1">
                    <div className="flex items-center justify-between text-slate-700 font-medium">
                      <span>{cat}</span>
                      <span className="font-bold text-slate-900">{count} cases ({pct}%)</span>
                    </div>
        ...
        <div className="p-6 rounded-2xl bg-white border border-slate-200/90 shadow-sm space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">Departmental SLA Compliance</h3>
          </div>
          ...
          <div className="divide-y divide-slate-100">
            {[
              ...
              <div key={agency.name} className="py-2.5 flex items-center justify-between gap-2">
                <div>
                  <span className="font-bold text-slate-800 block">{agency.name}</span>
                  <span className="text-[11px] text-slate-500">Target SLA: {agency.target}</span>
                </div>
                <div className="text-right">
                  <span className="font-bold text-emerald-600">{agency.resolved} on time</span>
                  <span className="text-[10px] text-slate-400 block">{agency.active} active incidents</span>
                </div>
              </div>
            ))}
          </div>
